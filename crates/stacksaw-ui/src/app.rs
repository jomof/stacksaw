...
use stacksaw_ssp::types::{Snapshot, Staircase};
...
impl App {
        ...
        let items: Vec<ListItem> = self
            ...
            .map(|(i, s)| {
                ...
                let line = Line::from(vec![
                    ...
                    RSpan::styled(s.name.clone(), Style::default().fg(color)),
        ...
        for (si, seg) in stair.segments.iter().enumerate() {
            ...
            for c in &seg.commits {
                ...
                let card = Line::from(vec![
                    RSpan::raw(format!("{indent}  ")),
                    RSpan::styled(c.short.clone(), Style::default().fg(color)),
                    RSpan::raw(format!(" {}", truncate(&c.subject, 40))),
    ...
    fn draw_files(&self, frame: &mut Frame, area: Rect) {
        frame.render_widget(Paragraph::new("(files of selected commit)"), area);
    }
